...
func (s *storage) saveInK8s(secret *v1.Secret) (*v1.Secret, error) {
	...
	if !s.initComplete() {
		...
		go s.initSync.Do(func() {
			err := wait.Poll(100*time.Millisecond, 15*time.Minute, func() (bool, error) {
				if !s.initComplete() {
					return false, nil
				}
			...
			if err != nil {
				logrus.Errorf("error while waiting for storage to initialized to save secret %s/%s: %v", secret.Namespace, secret.Name, err)
			}
		})
		return secret, nil
	}
